...
class MKVHandler(FileSystemEventHandler):
    ...
    def wait_until_file_is_ready(self, file_path):
        """ Wait until the file is fully written and closed. """
        while True:
            try:
...
class VLCPlayer:
    def __init__(self, master):
        ...
        self.frame.grid(row=5, column=0, columnspan=3, sticky='nsew')
        ...
        self.scale.set(100)
        self.scale.grid(row=6, column=0, columnspan=3, sticky='ew')
...
class DVDConverterApp:
    def __init__(self, master):
        ...
        logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(message)s', datefmt=smpte_timecode_format())
    ...
    def background_conversion(self, drive_index, mkv_output, mov_output):
        ...
        MKVHandler(mov_output, self.update_status_message, self.logger)
