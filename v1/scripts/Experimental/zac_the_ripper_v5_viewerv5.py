...
class FrameSampler(threading.Thread):
    ...
    def run(self):
        ...
        # Delay start to let some data build up
        time.sleep(10)  # 10 seconds, adjust as necessary
        while self.running:
            try:
                pipe = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=10**8)
                while True:
                    frame_data = pipe.stdout.readline()
                    if frame_data.strip():
...
class VLCPlayer:
    ...
    def display_frame(self, frame_data):
        media = self.instance.media_new_bytes(frame_data)
...
class DVDConverterApp:
    ...
    def initiate_ripping(self):
        ...
        if not disk_index.isdigit() or not mkv_output:
